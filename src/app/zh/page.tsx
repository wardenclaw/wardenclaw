...
const whyPoints = [
  {
    icon: '🔗',
    title: 'Agent 原生链上运行',
    desc: 'Warden Protocol 提供专为 AI Agent 设计的区块链基础设施，Agent 行为可验证、可追溯。',
  },
  {
    icon: '🧩',
    title: 'OpenClaw 标准化技能体系',
    desc: 'OpenClaw 为 Agent 提供统一的 Skill 标准，可跨平台复用和交易。',
  },
  {
    icon: '💰',
    title: '真实激励机制',
    desc: 'Agent Builder Programme 为开发者和内容创建者提供链上可追踪的激励。',
  },
  {
    icon: '🛡️',
    title: '安全与权限控制',
    desc: 'Warden 的 KeyChain 和 SpaceWard 提供细粒度权限管理，Agent 操作需授权。',
  },
];

const quickStartPaths = [
  { emoji: '📖', label: 'What — 什么是 Warden？', href: '/zh/what-is-warden', color: 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300' },
  { emoji: '🚀', label: 'How — 5 分钟部署 Agent', href: '/zh/guides/warden-studio-5min', color: 'bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300' },
  { emoji: '🤔', label: 'Why — OpenClaw 适合 Warden？', href: '/zh/openclaw-on-warden', color: 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300' },
  { emoji: '👤', label: 'Who — 适合哪些开发者', href: '/zh/guides/getting-started', color: 'bg-purple-50 text-purple-700 dark:bg-purple-950 dark:text-purple-300' },
];
...
const latestUpdates = [
  ...
  { date: '2026-03-15', title: 'OpenClaw × Warden 社区项目案例', href: '/zh/updates/2026-03-15' },
];
    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 -z-10">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full bg-warden-500/5 blur-3xl translate-x-1/3 -translate-y-1/3" />
          <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-claw-500/5 blur-3xl -translate-x-1/3 translate-y-1/3" />
        </div>

        <div className="container-wide py-20 sm:py-28 lg:py-36">
          <div className="max-w-3xl">
            <div className="inline-block mb-6 rounded-full bg-warden-50 px-4 py-1.5 text-xs font-medium text-warden-700 dark:bg-warden-950 dark:text-warden-300 fade-in">
              独立第三方知识库 · 每日更新
            </div>
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight text-ink-950 dark:text-ink-50 text-balance fade-in fade-in-delay-1">
              Warden Protocol +{' '}
            ...
            <p className="mt-6 text-lg text-ink-500 dark:text-ink-400 max-w-xl leading-relaxed fade-in fade-in-delay-2">
              帮助你理解底层协议、部署 Agent、参与激励、追踪生态动态。不夸张，不喊单，只提供结构化信息。
            </p>
      ...
      <section className="section-spacing border-t border-ink-100 dark:border-ink-800">
        <div className="container-wide">
          <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink-900 dark:text-ink-100 mb-10">
      ...
      {/* ══════════ LATEST UPDATES ══════════ */}
      <section className="section-spacing bg-ink-50/50 dark:bg-ink-900/30 border-t border-ink-100 dark:border-ink-800">
        <div className="container-wide">
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-display text-2xl font-semibold text-ink-900 dark:text-ink-100">最新动态</h2>
            <Link href="/zh/updates" className="text-sm text-warden-600 hover:text-warden-700 font-medium">
              查看全部 →
            </Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {latestUpdates.map((u) => (
              <Link key={u.date} href={u.href} className="card group">
                <time className="text-xs text-ink-400 font-mono">{u.date}</time>
                <h3 className="mt-2 text-sm font-semibold text-ink-800 group-hover:text-warden-600 transition-colors dark:text-ink-200 line-clamp-2">
                  {u.title}
                </h3>
              </Link>
            ))}
      ...
      <section className="section-spacing border-t border-ink-100 dark:border-ink-800">
        <div className="container-wide">
          <h2 className="font-display text-2xl font-semibold text-ink-900 dark:text-ink-100 mb-8">新手快速入门</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {quickStartPaths.map((p) => (
              <Link key={p.href} href={p.href} className="card group text-center">
                ...
                <span className={`inline-block rounded-full px-3 py-1 text-xs font-medium ${p.color}`}>
                  {p.label}
                </span>
              </Link>
            ))}
      ...
      <section className="section-spacing bg-ink-50/50 dark:bg-ink-900/30 border-t border-ink-100 dark:border-ink-800">
        <div className="container-wide">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/zh/guides" className="text-sm text-warden-600 hover:text-warden-700 font-medium">
              查看全部 →
            </Link>
          </div>
          <div className="space-y-3">
            {hotGuides.map((g, i) => (
              <Link key={g.href} href={g.href} className="card flex items-center gap-4 group">
                <span className="shrink-0 flex h-8 w-8 items-center justify-center rounded-lg bg-warden-50 text-xs font-bold text-warden-600 dark:bg-warden-950">
      ...
      <section className="section-spacing border-t border-ink-100 dark:border-ink-800">
        <div className="container-wide">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Link href="/zh/incentives/agent-builder-programme" className="card group">
            ...
            <Link href="/zh/incentives/warden-airdrop" className="card group">
              <div className="flex items-center gap-2 mb-2">
      ...
      <section className="section-spacing bg-ink-50/50 dark:bg-ink-900/30 border-t border-ink-100 dark:border-ink-800">
        <div className="container-page">
          ...
          <div className="space-y-4">
            {faqItems.map((f, i) => (
              <details key={i} className="group rounded-xl border border-ink-100 bg-white dark:border-ink-700 dark:bg-ink-800">
                <summary className="cursor-pointer px-6 py-4 text-sm font-semibold text-ink-800 dark:text-ink-200 list-none flex items-center justify-between">
                  {f.q}
                  <span className="ml-3 text-ink-300 group-open:rotate-45 transition-transform text-lg">+</span>
                </summary>
      ...
      {/* ══════════ RISK ══════════ */}
      <section className="py-12 border-t border-ink-100 dark:border-ink-800">
        <div className="container-page">
