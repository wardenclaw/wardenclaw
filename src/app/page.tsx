...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: 'https://wardenclaw.com',
    languages: { 'zh': 'https://wardenclaw.com/zh', 'en': 'https://wardenclaw.com/en' },
  },
};
      ...
      <main className="flex-1">
          ...
          <div className="container-wide relative py-24 sm:py-32 lg:py-40">
            <div className="max-w-3xl">
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-[3.5rem] font-bold leading-[1.15] text-ink-950 dark:text-white text-balance fade-in fade-in-delay-1">
                Warden Protocol +{' '}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-warden-500 via-warden-400 to-claw-500">OpenClaw</span>{' '}
                <br className="hidden sm:block" />完全指南
              </h1>
              ...
              <div className="mt-8 flex flex-wrap gap-3 fade-in fade-in-delay-3">
                <Link href="/zh/what-is-warden" className="btn-primary">开始阅读 <span aria-hidden>→</span></Link>
                <Link href="/zh/updates" className="btn-secondary">最新更新</Link>
              </div>
              <div className="mt-12 flex flex-wrap gap-8 text-sm fade-in fade-in-delay-4">
                <div><span className="block text-2xl font-bold text-ink-900 dark:text-white font-display">34+</span><span className="text-ink-400">深度页面</span></div>
                <div><span className="block text-2xl font-bold text-ink-900 dark:text-white font-display">7 天</span><span className="text-ink-400">日更示例</span></div>
                <div><span className="block text-2xl font-bold text-ink-900 dark:text-white font-display">中英</span><span className="text-ink-400">双语支持</span></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="section-spacing section-divider">
          <div className="container-wide">
            <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink-900 dark:text-ink-100 mb-3">为什么关注 Warden + OpenClaw？</h2>
        ...
        <section className="section-spacing bg-ink-50/50 dark:bg-[#111417] section-divider">
          <div className="container-wide">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href="/zh/updates" className="text-sm text-warden-600 hover:text-warden-700 dark:text-warden-400 font-medium">查看全部 →</Link>
            </div>
        ...
        <section className="section-spacing bg-ink-50/50 dark:bg-[#111417] section-divider">
          <div className="container-wide">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href="/zh/guides" className="text-sm text-warden-600 hover:text-warden-700 dark:text-warden-400 font-medium">查看全部 →</Link>
            </div>
            <div className="space-y-3">
              {hotGuides.map((g, i) => (
                <Link key={g.href} href={g.href} className="card flex items-center gap-4 group">
                  ...
                  <span className="flex-1 text-sm font-medium text-ink-800 group-hover:text-warden-600 transition-colors dark:text-ink-200">{g.title}</span>
            ...
            <div className="space-y-3">
              {faqItems.map((f, i) => (
                <details key={i} className="group rounded-xl border border-ink-100 bg-white dark:border-[#2a2d32] dark:bg-[#161a1e] transition-all hover:border-warden-200 dark:hover:border-warden-800">
                  ...
                  <div className="px-6 pb-4 text-sm text-ink-600 dark:text-ink-400 leading-relaxed">{f.a}</div>
                </details>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 section-divider">
          <div className="container-page"><RiskWarning locale="zh" /></div>
        </section>

        {/* CTA */}
        <section className="section-spacing section-divider">
          <div className="container-page">
            <CTA title="保持关注生态动态" description="我们每日追踪 Warden Protocol 和 OpenClaw 的重要变化，为你提供结构化、可信赖的信息。" primaryLabel="查看日更" primaryHref="/zh/updates" secondaryLabel="了解编辑原则" secondaryHref="/zh/about" />
          </div>
        </section>
      </main>
      <Footer locale="zh" />
